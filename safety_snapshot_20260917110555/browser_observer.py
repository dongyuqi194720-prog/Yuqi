...
def browser_click(selector):
    ...
    # ChatGPT is a protected send surface.
    # All ChatGPT sending must go through CodexBridge's
    # unique-marker visibility gate. Generic DOM clicks
    # are therefore forbidden on ChatGPT pages.
    page_url = str(page.get("url") or "").lower()
    if (
        page_url.startswith("https://chatgpt.com/")
        or page_url.startswith("https://chatgpt.com")
        or page_url.startswith("https://chat.openai.com/")
        or page_url.startswith("https://chat.openai.com")
    ):
        raise RuntimeError(
            "ChatGPT generic browser click forbidden; "
            "all sending must pass CodexBridge marker gate"
        )
