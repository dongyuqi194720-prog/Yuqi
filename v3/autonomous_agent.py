...
class AutonomousAgent:
    ...
    def __init__(
        ...
        self.state = {
            ...
            # V6.28.2：显式桌面命令只解析一次，防止队列消费后重复解析。
            "deterministic_computer_consumed": False,
            "computer_observe_done": False,
            "llm_decision_count": 0,
            # V6.26: 可观测 LLM 计数/耗时。
            # 统计所有真正发出的模型请求，便于闭环压力测试定位浪费。
            "llm_stats": {
                "total_calls": 0,
                "decision_calls": 0,
                "analysis_calls": 0,
    ...
    def _record_llm_call(self, kind, elapsed, prompt_chars=0, success=True):
        """V6.26：统一记录 LLM 调用，不参与业务决策。"""
        stats = self.state.setdefault("llm_stats", {})
        for _key in ("total_calls", "decision_calls", "analysis_calls", "completion_calls"):
        ...
        stats["total_seconds"] = float(stats.get("total_seconds", 0.0)) + float(elapsed)
        key = {
            "decision": "decision_calls",
            "analysis": "analysis_calls",
            "completion": "completion_calls",
        }.get(str(kind), None)
        if key:
    ...
    def get_llm_stats(self):
        """返回当前任务的 LLM 使用统计。"""
        stats = copy.deepcopy(self.state.get("llm_stats", {}))
        total = int(stats.get("total_calls", 0))
        seconds = float(stats.get("total_seconds", 0.0))
        stats["avg_seconds"] = round(seconds / total, 3) if total else 0.0
        return stats
...
NEXT_STEP_REQUIREMENT: 如果 TASK_COMPLETE=NO，给出下一轮最小、明确、可执行的步骤要求；如果 YES，写 NONE
...
"""
        ...
        _completion_start = time.time()
        try:
            response = self.codex.ask(prompt)
            self._record_llm_call(
                "completion",
                time.time() - _completion_start,
                len(prompt),
                True,
            )
        except Exception:
        ...
        remaining_match = re.search(
            r"REMAINING\s*:\s*(.+?)(?=\nNEXT_STEP_REQUIREMENT\s*:|$)",
            text,
            re.I | re.S
        )

        next_step_match = re.search(
            r"NEXT_STEP_REQUIREMENT\s*:\s*(.+)$",
            text,
            re.I | re.S
        )
    ...
    def ask_llm(
        ...
        def worker():
            ...
            try:
                ...
                # V6.29-R1：真实 LLM 调用入口。
                # 只有执行到 llm.invoke()，才算真正发生一次模型调用。
                llm_kind = (
                    "decision"
                    if llm is self.decision_llm
                    else "analysis"
                )

                print(
                    "V6.29 REAL LLM INVOKE:",
                    llm_kind,
                    "input_chars =",
                    len(str(prompt))
                )
        ...
        if result_type == "error":
            ...
            self._record_llm_call(
                "decision" if llm is self.decision_llm else "analysis",
                elapsed,
                len(str(prompt)),
                False,
            )
    ...
    def is_pure_computer_task(self, question):
        """
        V6.29-R2：
        判断当前任务是否为纯 COMPUTER / GUI 任务。

        只有纯桌面操作才能在 COMPUTER Queue 清空后直接完成。
        如果任务同时包含代码、文件、测试、验证、开发等要求，
        COMPUTER 完成后必须继续原任务状态机。
        """
    ...
    def local_text_click_action(self, question):
        """V6.29-R3.3-C：明确文字目标点击优先本地 OCR。"""
        import re
        ...
        # 坐标点击由 V6.28 deterministic_computer_action 处理。
        if re.search(
            r"(?:坐标|位置)\s*[\(（]?\s*\d+\s*[,，\s]\s*\d+",
            text,
            re.IGNORECASE
        ):
            return None
        ...
        return {
            ...
            "ACTION": "CLICK_TEXT_LOCAL",
            "ARGS": target,
            "REASON": "明确文字目标，优先本地 OCR bbox 点击",
            "ACTIONS": [],
        }

    def deterministic_computer_action(self, question):
        """V6.28: parse explicit desktop instructions without a Decision LLM.

        Only handles unambiguous commands whose target/arguments are present in
        the user's text. Unknown GUI intent still falls through to Decision LLM.
        Multiple explicit commands are returned as one executable queue.
        """
        import re
        ...
        click_re = re.compile(
            r"(?:点击坐标|鼠标点击|单击坐标|click\s*at)\s*[(（]?\s*(\d+)\s*[,，\s]\s*(\d+)\s*[)）]?",
            re.IGNORECASE,
        )
        type_re = re.compile(
            r"(?:输入|键入|type)\s*[“\"'](.+?)[”\"']",
        ...
        key_re = re.compile(
            r"(?:按\s*|press\s+)"
            r"(Enter|回车|Tab|Esc|Escape|Space|空格|Backspace|Delete|上|下|左|右|Ctrl\+[A-Za-z]|Alt\+[A-Za-z])"
            r"\b",
            re.IGNORECASE,
        )
        ...
        for m in click_re.finditer(text):
            found.append((m.start(), {
                ...
                "REASON": "用户明确给出坐标，确定性执行",
            }))
        for m in type_re.finditer(text):
            found.append((m.start(), {
                "ACTION": "KEYBOARD_TYPE",
                "ARGS": m.group(1),
                "REASON": "用户明确给出输入文本，确定性执行",
            }))
        for m in key_re.finditer(text):
            key = m.group(1)
            key_map = {"回车": "Return", "空格": "space", "Esc": "Escape", "上": "Up", "下": "Down", "左": "Left", "右": "Right"}
            found.append((m.start(), {
                "ACTION": "KEYBOARD_PRESS",
                "ARGS": key_map.get(key, key),
                "REASON": "用户明确指定按键，确定性执行",
            }))
        ...
        actions = [item[1] for item in found]
        first = actions[0]
        return {
            "TASK_CONTROL": "CONTINUE_STEP",
            "ACTION": first["ACTION"],
    ...
    def ask_decision(
        ...
        if count >= max_decisions:
            ...
            return (
                '{"TASK_CONTROL":"CONTINUE_STEP",'
                '"ACTION":"__DECISION_BUDGET_EXHAUSTED__",'
                '"ARGS":"",'
                '"REASON":"LLM Decision Budget exhausted; task completion not implied",'
                '"NEXT_STEP_REQUIREMENT":""}'
            )
...
"""
    ...
    def run(
        ...
        # V6.24: run() 后续兼容分支会局部 import json；
        # 在函数入口显式导入，保证前置确定性路由可安全使用。
        import json
        ...
        # V6.25：浏览器确定性搜索快速通道。
        # 用户明确要求“搜索/搜一下/查询”时，直接导航搜索引擎，
        # 不调用 Decision LLM；搜索结果再由真实浏览器状态验证。
        explicit_search = re.search(
            r"(?:搜索|搜一下|搜\s*|查询|search\s+)(?:一下|一下子)?[：: ]*[“\"']?(.+?)[“\"']?(?:$|。|，|\n)",
            str(question),
            re.IGNORECASE,
        )
        search_only_task = not any(
            word in str(question) for word in [
                "点击", "单击", "click", "输入", "填写", "然后", "再",
                "打开", "访问", "导航", "比较", "分析", "总结", "查找并"
            ]
        )
        if browser_task and explicit_search and search_only_task and not any(
            word in str(question) for word in ["代码搜索", "搜索源码", "search_code_index"]
        ):
            query = explicit_search.group(1).strip().strip('“”\"\' ')
            if query:
                try:
                    from tools.browser_observer import browser_search, browser_text
                    if browser_search(query):
                        time.sleep(0.4)
                        current_text = browser_text()
                        final_result = "已搜索：" + query + "\n" + str(current_text)[:12000]
                        print("V6.25 BROWSER SEARCH FAST PATH:", query)
                        self.state["previous_step_result"] = final_result
                        self.state["verify_result_done"] = True
                        self.state["verify_result_passed"] = True
                        self.state["phase"] = "DONE"
                    ...
                    print("V6.25 BROWSER SEARCH FAST PATH ERROR:", str(e))

        # V6.14：简单浏览器动作快速通道
        # 明确给出“点击某文字”时，先读取 DOM 元素并直接点击，
        # 不需要 GPT 决定点击哪个元素。
        explicit_click = re.search(
            r"(?:点击|单击|click)\s*[“\"']([^“”\"']+)[“\"']",
            str(question),
            re.IGNORECASE,
        )
        if browser_task and explicit_click and not explicit_search:
            target = explicit_click.group(1).strip()
            try:
                from tools.browser_observer import browser_elements, browser_click_text, browser_text
                elements = browser_elements()
                found = any(
                    str(e.get("text", "")).strip() == target
                    or str(e.get("aria", "")).strip() == target
                    for e in elements
                )
                if found and browser_click_text(target):
                    time.sleep(0.2)
                    current_text = browser_text()
                    final_result = "已点击：" + target + "\n" + str(current_text)[:12000]
                    print("V6.25 BROWSER CLICK FAST PATH:", target)
                    self.state["previous_step_result"] = final_result
                    self.state["verify_result_done"] = True
                    self.state["verify_result_passed"] = True
                    self.state["phase"] = "DONE"
                ...
                print("V6.25 BROWSER CLICK FAST PATH ERROR:", str(e))
        ...
        for step in range(
            ...
            if (
                ...
                path_match = re.search(
                    ...
                    r"(?<![\w/.-])([\w.-]+\.(?:py|cpp|cc|c|h|hpp))",
                    question
                )

                candidate = None
                if path_match:
                    absolute_candidate = path_match.group(1)
                    relative_candidate = path_match.group(2)

                    if (
                        absolute_candidate
                        and os.path.isfile(absolute_candidate)
                    ):
                        candidate = absolute_candidate
                    elif relative_candidate:
            ...
            # V6.24: SEARCH 是确定性工具路由，不再先调用 LLM 再强制 SEARCH。
            # search_code_index 本身只需要关键词/任务文本，不需要模型决定“是否搜索”。
            # 这样普通代码任务的第一轮至少节省 1 次 LLM。
            deterministic_search = (
                self.state.get("phase") == "SEARCH"
                and self.state.get("task_mode") in {"REVIEW", "CHANGE"}
            )
            ...
            if deterministic_search:
                response = json.dumps({
                    ...
                    "ACTION": "SEARCH",
                    "ARGS": "",
                    "REASON": "SEARCH 阶段由 Python 状态机确定性执行",
                    "NEXT_STEP_REQUIREMENT": ""
                }, ensure_ascii=False)
                prompt = ""
                print("V6.24 DETERMINISTIC SEARCH → skip LLM")
            elif self.state.get("phase") == "COMPUTER":
            ...
            # V6.23：ANALYZE 完成后由 Python 状态机确定性进入 VERIFY。
            # ANALYZE 不再经过 Decision Layer，避免 3B 模型输出无效 ACTION
            # 导致 ANALYZE -> ANALYZE 死循环。
            if self.state.get("phase") == "ANALYZE" and self.state.get("analyze_done"):
                self.state["phase"] = "VERIFY"
                print("V6.23 ANALYZE -> VERIFY deterministic")
                continue
...
"""
            ...
            if self.state["phase"] == "VERIFY_RESULT":
                ...
                # V6.24：只有本轮真实验证通过，才需要让 GPT 判断“整个任务”
                # 是否完成以及下一步是什么。验证失败时无需再做一次完成判断，
                # 直接进入下一轮修复闭环即可，避免无意义的额外 LLM 调用。
                if self.state.get("verify_result_passed", False):
                    completion = self.check_task_completion()
                else:
                    completion = {
                        "complete": False,
                        "progress": "本轮修改已执行，但最终验证未通过",
                        "remaining": "修复本轮修改并重新验证",
                        "next_step_requirement": "修复本轮修改失败的问题并重新验证",
                        "raw": ""
                    }
                ...
                self.state["next_step_requirement"] = completion.get("next_step_requirement", "").strip()

                # GPT 若未给出专门的下一步字段，使用 REMAINING 作为确定性兜底。
                if (
                    not self.state["task_complete"]
                    and not self.state["next_step_requirement"]
                    and self.state["remaining_requirements"]
                    and self.state["remaining_requirements"].upper() != "NONE"
                ):
                    self.state["next_step_requirement"] = self.state["remaining_requirements"]
                ...
                # V6.24：VERIFY_RESULT 后不再再调用一次本地 Decision LLM。
                # check_task_completion() 已经由 GPT 普通聊天完成：
                #   1. 整体任务是否完成
                #   2. 未完成时下一轮最小步骤要求
                # 因此这里直接使用同一次 GPT 结果推进状态机。
                if self.state.get("verify_result_passed", False):
                    next_requirement = str(
                        self.state.get("next_step_requirement", "")
                    ).strip()
                    ...
                    if next_requirement:
                        ...
                        try:
                            step_number = int(current_step.rsplit("-", 1)[1])
                        except Exception:
                            step_number = 1
                        next_step = "STEP-" + str(step_number + 1).zfill(3)

                        self.state["task_control"] = "NEXT_STEP"
                        self.state["previous_step_result"] = str(self.state.get("verify_result", ""))
                        self.state["current_step_id"] = next_step
                        self.state["current_step_requirement"] = next_requirement

                        print("V6.24 STEP ADVANCE (single GPT completion call):", current_step, "->", next_step)
                        ...
                        self.state["verify_result_passed"] = False
                        self.state["verify_result"] = ""
                        self.state["next_step_requirement"] = ""
                        self.state["phase"] = "READ"
                        continue
            ...
            #
            # V6.28.4：
            # Queue 中的动作已经在上一轮确定性解析完成。
            # 当前动作直接进入 computer_tools dispatch，
            # 不允许再次进入 Decision Layer。
            if queued_action is None and self.state.get("computer_action_queue"):
                ...
                self.state["computer_queue_direct_dispatch"] = True

                print(
                    "V6.28.5 COMPUTER Queue DIRECT →",
                    queued_action.get(
                        "ACTION",
                    ...
                    queued_action.get(
                        "ARGS",
                        queued_action.get("args", "")
                    )
                )

            # V6.29-R3.3-C：
            # 明确“找到文字目标并点击”时，优先本地 OCR bbox。
            deterministic_local_click = None

            if (
                queued_action is None
                and self.state.get("phase") == "COMPUTER"
                and not self.state.get(
                    "deterministic_computer_consumed",
                    False
                )
            ):
                deterministic_local_click = (
                    self.local_text_click_action(question)
                )

                if deterministic_local_click:
                    self.state["deterministic_local_click"] = True
                    self.state["deterministic_local_click_failed"] = False
                    self.state["deterministic_computer_consumed"] = True
                    ...
                    print(
                        "V6.29-R3.3-C LOCAL TEXT CLICK →",
                        deterministic_local_click.get(
                            "ARGS",
                            ""
                        )
                    )

            # V6.28：显式桌面命令直接执行，禁止进入 Decision LLM。
            deterministic_computer = None
            if (
                queued_action is None
                and self.state.get("phase") == "COMPUTER"
                and not self.state.get("deterministic_computer_consumed", False)
            ):
                deterministic_computer = self.deterministic_computer_action(question)

                # V6.28.2：本轮 question 的确定性桌面命令只解析一次。
                self.state["deterministic_computer_consumed"] = True
                if deterministic_computer:
                    response = json.dumps(deterministic_computer, ensure_ascii=False)
                    prompt = ""
                    print(
                        "V6.28 DETERMINISTIC COMPUTER →",
                        deterministic_computer.get("ACTION"),
                        deterministic_computer.get("ARGS", "")
                    )
            ...
            # V6.27：开发状态机阶段动作本身已经由状态机唯一确定。
            # READ / VERIFY / MODIFY_PLAN / PLAN_VERIFY / MODIFY / VERIFY_RESULT
            # 不再先调用一次 Decision LLM 再被后面的状态机强制覆盖。
            # MODIFY 阶段仍会在真正需要生成完整源码时调用一次执行 LLM；
            # 这里跳过的是“多余的下一步决策”调用。
            deterministic_phase_action = None
            if queued_action is None and not deterministic_search:
                phase_actions = {
                    "READ": "READ",
                    "VERIFY": "VERIFY",
                ...
                phase_action = phase_actions.get(
                    str(self.state.get("phase", "")).upper()
                )
                if phase_action:
                    deterministic_phase_action = phase_action
                    response = json.dumps({
                        "TASK_CONTROL": "CONTINUE_STEP",
                        "ACTION": phase_action,
                        "ARGS": "",
                        "REASON": "当前状态机阶段已经唯一确定下一动作，跳过 Decision LLM",
                        "NEXT_STEP_REQUIREMENT": "",
                    }, ensure_ascii=False)
                    prompt = ""
                    print("V6.27 DETERMINISTIC PHASE →", phase_action, "skip decision LLM")

            if queued_action is not None:
                # 确定性动作已经生成，禁止再次调用 LLM。
                pass
            elif deterministic_local_click:
                # V6.29-R3.3-C：本地 OCR 点击已经确定。
                pass
            elif deterministic_computer:
                # V6.28：显式桌面命令已确定，禁止再次调用 Decision LLM。
                pass
            elif deterministic_search:
                # SEARCH 的 ACTION 由 Python 已确定，不经过 Decision Layer。
                print("V6.24 SEARCH: deterministic action, skip decision LLM")
            elif deterministic_phase_action:
                # V6.27：阶段动作已由状态机唯一确定，保持上面生成的响应，
                # 绝不落入 Decision LLM。
                print("V6.27 deterministic phase action: skip decision LLM")
            ...
            if deterministic_local_click:
                ...
                print(
                    "V6.29-R3.3-C LOCAL CLICK decision DIRECT →",
                    decision["args"],
                    "skip Decision LLM"
                )
                ...
                # V6.28.3：
                # 当前任务由用户的显式 COMPUTER 指令完全确定。
                # 例如：点击坐标 -> 输入文本 -> 按回车。
                # 这类任务完成动作队列后直接结束，不再进入
                # ANALYZE / VERIFY / Decision LLM。
                self.state["deterministic_computer_task"] = True
                self.state["deterministic_computer_failed"] = False

                decision = {
                    "allowed": True,
                    "action": deterministic_computer.get("ACTION", ""),
                    "args": deterministic_computer.get("ARGS", ""),
                    "task_control": deterministic_computer.get("TASK_CONTROL", "CONTINUE_STEP"),
                    "actions": deterministic_computer.get("ACTIONS", [])
                }

            # V6.28.4：
            # Queue 后续动作已经确定，直接构造 decision。
            # 这里是 Queue 与统一 computer_tools dispatch 的桥接点。
            # 绝不能落入 decision_step()。
            elif queued_action is not None:
                decision = {
                    "allowed": True,
                    "action": str(
                        queued_action.get(
                ...
                print(
                    "V6.28.5 COMPUTER Queue decision DIRECT →",
                    decision["action"],
                    "ARGS=",
                    decision["args"],
                    "skip Decision LLM"
                ...
                decision = {
                    "allowed": True,
                    "action": deterministic_phase_action,
                    "args": "",
                    "task_control": "CONTINUE_STEP",
                    "actions": []
                }
            elif self.state.get("phase") == "ANALYZE":
                # V6.23：ANALYZE 已由 Python 确定性生成 analyze_code，
                # 不再把内部工具协议送入 Decision Layer。
                decision = {
                    "allowed": True,
                    "action": "ANALYZE",
                    "args": ""
            ...
            computer_tools = {
                ...
                "CLICK_TEXT_LOCAL": "click_text_local",
            }
            ...
            if (
                ...
                for item in decision_actions[1:]:
                    # V6.28.2：
                    # deterministic_computer_action() 使用协议大写键
                    # ACTION / ARGS / REASON；
                    # Decision LLM 旧协议可能使用小写键。
                    # 两种格式统一兼容，避免确定性 ACTIONS 丢失。
                    action_name = str(
                    ...
                    if action_name in computer_tools or action_name == "DONE":
                        valid_queue.append({
                            "ACTION": action_name,
                            "ARGS": str(
                                item.get("ARGS", item.get("args", ""))
                            ).strip(),
                            "REASON": str(
                                item.get("REASON", item.get("reason", ""))
                            ),
                            "TASK_CONTROL": (
                                "DONE"
                                if action_name == "DONE"
            ...
            if (
                ...
                # V6.28.2：
                # COMPUTER 队列中的后续动作不能覆盖最初的返回阶段。
                # 只有从非-COMPUTER 阶段首次进入 COMPUTER 时，
                # 才记录 computer_return_phase。
                if self.state.get("phase") != "COMPUTER":
                    self.state["computer_return_phase"] = self.state.get(
                        "phase",
                        "ANALYZE"
                    )
            ...
            if (
                ...
                and decision.get("action") != "ANALYZE"
            ):
            ...
            if tool == "read_file_chunk":
                ...
                if read_path == self.project:
                    ...
                    if self.state["target_file"]:
                        ...
                        self.state["decision_request"] = self.build_decision_request(
                            observation="READ 被安全层拒绝：没有有效 target_file"
                        )
                        continue
            ...
            if tool in computer_tool_names:
                ...
                # V6.28.4：
                # 当前 Queue direct-dispatch 已完成。
                # 标记只属于当前一次 dispatch，立即清除。
                self.state["computer_queue_direct_dispatch"] = False
        ...
        "窗口激活失败：",
                ...
                # 队列还有动作时，不返回 GPT，不重新决策。
                if self.state.get("computer_action_queue"):
                    self.state["phase"] = "COMPUTER"
                    print(
                        "V6.28.6 COMPUTER Queue remaining →",
                        len(self.state["computer_action_queue"]),
                        "actions, direct dispatch, skip Decision Layer"
                    )
                    continue

                # 当前 COMPUTER 动作序列已经全部执行完毕。
                #
                # V6.28.3：
                # 如果整个任务本身就是用户明确给出的确定性 COMPUTER
                # 动作序列，那么这里已经达到任务完成条件。
                #
                # 例如：
                #   点击坐标 -> 输入文本 -> Enter
                #
                # 不再：
                #   COMPUTER -> ANALYZE -> VERIFY -> LLM
                #
                # 这样可以保证确定性 GUI 指令真正做到零额外 LLM。
                if (
                    self.state.get("deterministic_computer_task")
                    and not self.state.get("deterministic_computer_failed")
                    ...
                    print(
                        "V6.28.3 COMPUTER deterministic task COMPLETE"
                    )
                    print(
                        "V6.28.3 COMPUTER actions finished:",
                        "no further LLM"
                    )
                ...
                # V6.29-R3.3-C：
                # 本地文字点击成功后，纯 COMPUTER 任务直接完成。
                if (
                    self.state.get("deterministic_local_click")
                    and not self.state.get(
                        "deterministic_local_click_failed"
                    )
                    and self.is_pure_computer_task(question)
                ):
                    self.state["task_complete"] = True
                    ...
                    print(
                        "V6.29-R3.3-C LOCAL TEXT CLICK COMPLETE:",
                        "no Decision LLM"
                    )
                ...
                print(
                    "V6.28.2 COMPUTER Result → next state:",
                    self.state["phase"]
                )
