...
class CodexBridge:
    ...
    def ask(self, prompt):
        ...
        with sync_playwright() as p:
            ...
            box.focus()
            box.press("Control+A")
            page.keyboard.insert_text(send_text)

            try:
                visible_text = box.inner_text(
                    timeout=3000
                )
            except Exception as exc:
                raise RuntimeError(
                    "Cannot verify ChatGPT input box; sending forbidden"
                ) from exc
            ...
            while time.time() < deadline:
                ...
                if total_count <= before_assistant_count:
                    continue
